...
export function renderPotShopPreview(shape: string, colorId: string): string {
  ...
  if (shape === 'conic') {
    ...
    potPath = `<path d="M3,${potTop} L33,${potTop} L29,${potTop + potH} L7,${potTop + potH} Z" fill="${c.body}"/>`
    ...
    potPath = `<rect x="11" y="${potTop}" width="14" height="${potH}" rx="4" fill="${c.body}"/>`
...
export function renderPot(w: number, groundY: number, potRimH: number, potH: number, body: string, potDesign?: PotDesign): string {
  ...
  if (shape === 'conic') {
    ...
    body += `<path d="M${tXL},${potTop} L${tXR},${potTop} L${bXR},${potBot} L${bXL},${potBot} Z" fill="${c.body}"/>`;
    ...
    body += `<rect x="${tinyPotX}" y="${potTop}" width="${tinyPotW}" height="${potH}" rx="6" fill="${c.body}"/>`;
