...
import { expressedColor, expressedNumber, expressedShape, expressedGradient, dominantShape, dominantCenter } from "./genetic.utils";
...
export function computeBreedEstimate(a: Plant, b: Plant): BreedEstimate {
  ...
  // Petal count: derive range from parent averages + jitter
  ...
  // Color: sample 30 children for realistic hue spread
  ...
  // Gradient probability from samples
  const gradCount = samples.filter(p => expressedGradient(p.gradientColor) !== null).length

  return {
    midH: avgH,
    minH: (avgH + 338) % 360,  // ~22° below
    maxH: (avgH + 22) % 360,   // ~22° above
    minP,
    maxP,
